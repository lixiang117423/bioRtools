...
### Added
- `plot_cim_lod()`: Manhattan-style LOD score plot for CIM/QTL scan results.
  Computes cumulative physical positions across chromosomes with alternating
  color scheme, configurable position scaling (Mbp→bp), LOD offset for
  zero-handling, and optional genome-wide threshold line. Accepts data.frame,
  data.table, or file path as input. Returns a list with the ggplot object,
  processed data, and chromosome center positions.
...
### Added
- `read_data()`: GFF3 support (`.gff`/`.gff3`). Directive/comment lines (those starting with `#`) are dropped and the nine standard columns (`seqid`…`attributes`) are returned with fixed names; column 9 is kept as the raw `key=value;...` string and `.` is read as `NA`. Comment lines are filtered by line-start rather than via readr's `comment` argument, which would otherwise truncate any `#` inside an attribute value. No new dependencies.
